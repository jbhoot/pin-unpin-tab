...
let wait_until_element_scroll ev =
  let%lwt (_ : Dom_html.event Js.t) = Lwt_js_events.scroll (Dom_html.eventTarget ev) in
  Lwt.return WaitUntilDocumentScroll
          ...
          Lwt_js_events.mousedowns 
            ...
            (fun ev _ -> 
              if is_valid_click ev then
                let%lwt waited = Lwt.pick [wait_until_timeout (config##.longClickToggleTime /. 1000.); wait_until_mouse_up (); wait_until_mouse_move (); wait_until_document_scroll (); wait_until_element_scroll ev] in
                  match waited with
